...
double read_num_field(lua_State* L, int idx, const char* field) {
    ...
    if (!lua_isnumber(L, -1)) {
        std::string msg = std::format(
            "moused: expected number field `{}` in script command table",
            field);
        lua_pop(L, 1);
...
macro_script parse_lua_result(lua_State* L) {
    ...
    if (top < 1 || !lua_istable(L, -1)) {
        throw std::runtime_error(
            "moused: hotkey script must return a table of commands");
    }
    ...
    try {
        ...
        for (std::size_t i = 1; i <= n; ++i) {
            ...
            if (!lua_istable(L, -1)) {
                throw std::runtime_error(std::format(
                    "moused: element #{} of script result is not a table", i));
            }

            command cmd;
            cmd.type = static_cast<command_type>(
                static_cast<int>(read_num_field(L, -1, "cmd")));
            ...
            if (!lua_istable(L, -1)) {
                throw std::runtime_error(std::format(
                    "moused: command #{} is missing a table `args`", i));
            }
            ...
            for (std::size_t a = 1; a <= args_len; ++a) {
                ...
                if (!lua_isnumber(L, -1)) {
                    throw std::runtime_error(std::format(
                        "moused: arg #{} of command #{} is not a number", a,
                        i));
                }
        ...
        if (script.empty())
            throw std::runtime_error(
                "moused: hotkey script returned an empty command list");
    } catch (...) {
        // restore the stack so repeated calls on the same state don't
        // accumulate
        lua_settop(L, top);
        throw;
    }
