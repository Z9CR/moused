...
#endif
#ifdef DELETE
#undef DELETE  // Windows.h defines DELETE as 0x00010000L — we need it for
               // keys::DELETE
#endif
#include <cmath>
...
bool is_key_pressed(keys key) {
    ...
    if (val > 0 && val < 256) {
        ...
        // F1 – F12
        if (!vk && val >= 290 && val <= 301) vk = VK_F1 + (val - 290);

        // Numpad 0–9
        if (!vk && val >= 320 && val <= 329) vk = VK_NUMPAD0 + (val - 320);
