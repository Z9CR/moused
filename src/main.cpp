...
bool moused::OnInit()
{
    try
    {
        ...
        g_listener = std::jthread([](std::stop_token st)
                                  {
            std::vector<char> prev(keys_properties.size(), 0); // combo fully-pressed last round
            while (!st.stop_requested())
            {
                ...
                for (std::size_t i = 0; i < keys_properties.size(); ++i)
                {
                    ...
                    if (!prop.enabled || prop.keys.empty())
                        continue;
                    bool all = true;
                ...
                std::sort(rising.begin(), rising.end(),
                          ...
                          { return keys_properties[a].keys.size() > keys_properties[b].keys.size(); });
                ...
                for (std::size_t idx : rising)
                {
                    ...
                    bool blocked = false;
                    for (auto k : combo)
                        if (std::find(occupied.begin(), occupied.end(), k) != occupied.end())
                        {
                            blocked = true;
                            break;
                        }
