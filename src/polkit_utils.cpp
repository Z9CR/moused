...
void polkit_root_getter(int argc, char* argv[]) {
    ...
    // Look for saved environment from the original user.
    {
        ...
        if (sudo_uid)
            orig_uid = (uid_t)atoi(sudo_uid);
        else if (pkexec_uid)
            orig_uid = (uid_t)atoi(pkexec_uid);
        ...
        if (orig_uid != (uid_t)-1) {
            ...
            // For simplicity, try the most likely uid (1000 is common first
            // desktop user)
            ...
            while (std::getline(f, line)) {
                ...
                if (eq != std::string::npos) {
                    ...
                    if (key == "MU_ORIG_UID") {
                        ...
                        // Restore environment variable (overwrite pkexec's root
                        // env with the original user's values)
